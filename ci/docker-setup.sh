...
docker run --restart=always  -p 30024:22  -d --network-alias=sftp_private --name=sftp_private  -v sftpkey:/home/foo/  -v ./sshpublic.key:/home/foo/.ssh/keys/id_rsa.pub:ro  docker.io/atmoz/sftp foo::1001:0:/home/foo/upload


###ssh
#docker run --restart=always  -p 32768:22  -d --network-alias=sshd --name=sshd  -d docker.io/sickp/centos-sshd
docker run --restart=always  -p 32768:22  -d --network-alias=sshd --name=sshd  -d rastasheep/ubuntu-sshd:14.04
